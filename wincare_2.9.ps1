...
function Run-DebloaterMenu {
    ...
    function Run-StoreMenu {
        Write-Host "`n=== Manage Microsoft Store ===" -ForegroundColor Cyan
        
        # Check current status
        $storePkg = Get-AppxPackage -Name "Microsoft.WindowsStore"
        $status = if ($storePkg) { "INSTALLED" } else { "MISSING" }
        $col = if ($storePkg) { "Green" } else { "Red" }
        Write-Host "   Status: [$status]" -ForegroundColor $col
        
        Write-Host "`n   1. Restore / Reinstall (Register XML)"
        Write-Host "   2. Uninstall Microsoft Store"
        Write-Host "   0. Back"
        
        $action = Read-Host "Select Option"
        
        if ($action -eq "1") {
            Write-Host "Phase 1: Registering XML Manifests..." -ForegroundColor Yellow
            try {
        ...
        elseif ($action -eq "2") {
            ...
            Get-AppxPackage -AllUsers *WindowsStore* | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
            Get-AppxPackage -AllUsers *StorePurchaseApp* | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
            Write-Status "[OK] Removal command executed." "Green"
            Pause
        }
    }
    ...
    do {
        ...
        Write-Host "8.  Manage Microsoft Store (Restore / Remove)"
...
function Show-Menu {
    Clear-Host
    Write-Host "========== WinCare v2.9 ==========" -ForegroundColor Cyan
    Write-Host ""
