...
<#
    ...
    Updated: 2026-02-23
    Description: Standalone tool for deep Windows Telemetry, AI (Recall/Copilot/Intelligence), and Ads management.
#>
...
function Get-SystemStatus {
    ...
    try {
        ...
        if ($rPol -eq 1) { $recallStatus = "Disabled" }
    } catch {}
...
function Apply-TelemetryFix {
    ...
    # 1. Services
    # PcaSvc (Program Compatibility Assistant) collects app usage telemetry, highly recommended to disable for deep privacy
    $services = @("DiagTrack", "dmwappushservice", "diagnosticshub.standardcollector.service", "diagsvc", "wersvc", "wercplsupport", "PcaSvc")
...
function Apply-AIFix {
    ...
    # 1. Recall Optional Feature (Windows 11 24H2+)
    try {
    ...
    # 2. Windows AI Policy (Blocks AI snapshotting and analysis)
    ...
    # 3. Windows Intelligence (New in 2025/2026)
...
function Apply-AdsFix {
    ...
    # 2. Settings App Suggestions & Nags ("Finish setting up your device")
